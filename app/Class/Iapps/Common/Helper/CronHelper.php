...
class CronHelper {
    ...
    private static function isrunning($lock_name = 'cronHelper') {
        ...
        if (in_array(self::$pid, $pids)) {
            log_message("debug", "pids = " . json_encode($pids) . "|" . self::$pid . PHP_EOL);
            return TRUE;
        }
        return FALSE;
    }

    private static function isrunningCli($lock_name = 'cronHelper') {
        $pids = explode(PHP_EOL, `ps -e |grep " common Batch_cli $lock_name " |awk '{print $2}'`);
        log_message("debug", "pids count = " . count($pids) . PHP_EOL);
        if (is_array($pids) && count($pids) > 6) {
            log_message("debug", "pids count = " . count($pids) . PHP_EOL);
            return TRUE;
        }
        return FALSE;
    ...
    private static function isrunningCurl($route = 'job/default') {
    ...
    public static function unlock($lock_name = 'cronHelper') {
        global $argv;
        ...
        log_message("debug", "==" . self::$pid . "== Releasing lock...");
        return TRUE;
    }
